...
function createPlanMonthDisplayUi({
  appSettings,
  currentPeriod,
  fiscalMonths,
...
function syncPlanSettingsTableColumnPlates(tableWrap, table, highlightMonth, fiscalMonths) {
...
function syncAllPlanSettingsTableColumnPlates(wrap, fiscalMonths, currentPeriod, appSettings) {
...
function refreshPlanSettingsColumnPlates(wrap, fiscalMonths, currentPeriod, appSettings) {
  if (!wrap?.isConnected) return;
...
function bindPlanSettingsColumnPlateSync(wrap, fiscalMonths, currentPeriod, appSettings) {
...
function bindPlanSettingsTableUi(wrap, fiscalMonths, currentPeriod, appSettings) {
...
function planSettingsRemToPx(rem) {
  const base = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16;
  return rem * base;
}

/** 要素を一時的に max-content 幅にして自然幅を計測する（折り返し・伸縮の影響を除外） */
function measureElementIntrinsicWidth(el) {
 ...
 */
function layoutPlanSettingsAutoZoom(wrap, measureNaturalWidth) {
  if (!wrap?.isConnected) return;
 ...
 */
function bindPlanSettingsAutoZoom(wrap, measureNaturalWidth) {
  const host = wrap.parentElement;
  if (!host) return;
...
function layoutEmployeeSettingsTables(wrap, monthCount) {
...
function bindEmployeeSettingsLayout(wrap, {
  monthCount,
  fiscalMonths,
  currentPeriod,
  appSettings,
...
function resetPlanSettingsUiScale(wrap) {
...
function measurePlanSettingsTablesNaturalWidth(wrap, tableSelector = 'table') {
  let maxNaturalW = 0;
...
function layoutPlanSettingsScalableWrap(wrap, measureNaturalWidth) {
